USE [PART0]
GO
/****** Object:  StoredProcedure [SK_F2_FLUSSI].[F2_EXP_OrganiSociali]    Script Date: 06/02/2018 10:21:53 ******/
IF EXISTS (SELECT *
             FROM sys.objects
            WHERE OBJECT_ID = OBJECT_ID(N'[SK_F2_FLUSSI].[F2_EXP_OrganiSociali]')
              AND TYPE IN (N'P', N'RF', N'PC'))
BEGIN
    DROP PROCEDURE SK_F2_FLUSSI.F2_EXP_OrganiSociali;
END
...
-- ********************************************************************************************************************************
/*
Parametro dest: indica il destinatario per cui viene generato export
Valori: CI,FITP,OPA
*/
CREATE PROCEDURE [SK_F2_FLUSSI].[F2_EXP_OrganiSociali]
@dataEstrazione date, @dest char(5), @outputNum int OUTPUT, @outputMsg nvarchar(500) OUTPUT
WITH EXEC AS CALLER
AS
BEGIN
  ...
  declare @pesoOrgano			int
  declare @pesoIncarico			int
  ...
  --BEGIN
      ...
      DELETE FROM SK_F2_FLUSSI.F2_T_EXP_OrganiSociali
       WHERE Data_estrazione = @dataEstrazione
	         AND Destinatario = @dest
      ...
      BEGIN    
        ...
        DECLARE Esponenti_CUR CURSOR FOR
         SELECT distinct esp.ID_Persona_Fisica, ISNULL(esp.ID_Societa_Gruppo,0), org.ID_Organo, inc.ID_Incarico,
				...
				orgd.descrizione as descrizione_organo,
				incd.descrizione as descrizione_incarico,
				orgd.Peso as peso_organo,
				orginc.Peso as peso_incarico
           FROM SK_F2.F2_T_Organi_Societa org, SK_F2.F2_T_Incarichi_Societa inc,
				...
				[SK_F2].[F2_D_Organi] orgd,[SK_F2].[F2_D_Organi_Incarichi] orginc,[SK_F2].[F2_D_Incarichi] incd
          WHERE org.ID_Operazione = @idOperazione
			...
			AND orgd.ID = org.ID_Organo
            AND (inc.Cancellata IS NULL or inc.Cancellata = 0)
            AND esp.ID_Incarico_Societa = inc.ID
			AND orginc.ID_Incarico = inc.ID_Incarico
			AND incd.ID = orginc.ID_Incarico
        ...
        OPEN Esponenti_CUR
        FETCH NEXT FROM Esponenti_CUR 
				INTO @idPF, @idSocietaGruppo, @idOrgano, 
					@idIncarico,@esecutivoFitProper,@indipendenza, 
					@codMacrocat, @descMacrocat, @dataDecorrenza, 
					@dataFine, @motivoCessazione,@descOrgano,@descIncarico,@pesoOrgano,@pesoIncarico
        ...
        BEGIN 
            ...
            INSERT INTO SK_F2_FLUSSI.F2_T_EXP_OrganiSociali (
               Data_estrazione
              ,ABI_Segnalante
              ,SNDG_Partecipata
              ,Codice_Istituto
            ...
            FETCH NEXT FROM Esponenti_CUR INTO @idPF, @idSocietaGruppo, @idOrgano, @idIncarico, 
						...
						@descOrgano,@descIncarico,@pesoOrgano,@pesoIncarico
        END
        CLOSE Esponenti_CUR
      ...
      -- Gestione scarti
      UPDATE SK_F2_FLUSSI.F2_T_EXP_OrganiSociali
         SET Flag_Scarto = 1,
	  ...
	  BEGIN
		...
		BEGIN
			SELECT Codice_Istituto +
             ...
             left(Descrizione_Incarico + space(50), 50) +
             left(Descrizione_Organo + space(50), 50)
				FROM SK_F2_FLUSSI.F2_T_EXP_CI_OrganiSociali
				WHERE Data_estrazione = @dataEstrazione
				-- AND ABI_Segnalante = @abi
				AND Flag_Scarto = 0 
	  END
	   ...
	   BEGIN
		...
		ELSE
			BEGIN
				...
				UPDATE SK_F2_FLUSSI.F2_T_OrganiSociali
				SET Flag_Scarto = 5,
					Motivo_Scarto = CONCAT('Destinatario non riconosciuto: ',@dest)
			WHERE Data_estrazione = @dataEstrazione
	   END      
 END TRY
	BEGIN CATCH
