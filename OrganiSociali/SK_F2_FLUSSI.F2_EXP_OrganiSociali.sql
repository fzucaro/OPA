...
@dataEstrazione date, @destinatario nvarchar(5), @outputNum int OUTPUT, @outputMsg nvarchar(500) OUTPUT
WITH EXEC AS CALLER
AS
BEGIN
  ...
  BEGIN TRY
	...
	-- eliminata logica segnalante che utilizza abi
      ...
      BEGIN    
        ...
        BEGIN 
            ...
            SELECT @descOrgano = ISNULL(o.Descrizione,''),
                   @pesoOrgano = ISNULL(o.Peso,0)
              FROM SK_F2.F2_D_Organi o
             WHERE o.ID = @idOrgano
            set @descOrgano = ISNULL(@descOrgano,'')
            set @pesoOrgano = ISNULL(@pesoOrgano,0)
            ...
            -- Ricavo Peso Incarico
            set @pesoIncarico = 0
            SELECT @pesoIncarico = ISNULL(oi.Peso,0)
              FROM SK_F2.F2_D_Organi_Incarichi oi
             WHERE oi.ID_Organo = @idOrgano
               AND ID_Incarico = @idIncarico
            set @pesoIncarico = ISNULL(@pesoIncarico,0)
            ...
            INSERT INTO SK_F2_FLUSSI.F2_T_EXP_OrganiSociali (
              ...
              ,@destinatario -- Sistema destinatario nvarchar(5)
            ...
            FETCH NEXT FROM Esponenti_CUR INTO @idPF, @idSocietaGruppo, @idOrgano, @idIncarico, 
						                                   @esecutivoFitProper, @indipendenza, @codMacrocat, @descMacrocat, 
                                               @dataDecorrenza, @dataFine, @motivoCessazione
        END
        CLOSE Esponenti_CUR
    ...
    -- Select Finale da tabella in base a destinatario
    IF @destinatario = 'OPA'
    BEGIN
  	  SELECT left(Codice_Istituto + space(2), 2) +
             ...
             right('     ' + Ltrim(Rtrim(convert(nvarchar, Peso_Organo))), 5) +
             right('     ' + Ltrim(Rtrim(convert(nvarchar, Peso_Incarico))), 5)
		    FROM SK_F2_FLUSSI.F2_T_EXP_OrganiSociali
		   WHERE Data_estrazione = @dataEstrazione
		     AND Destinatario = @destinatario
		     AND Flag_Scarto = 0    
    END
